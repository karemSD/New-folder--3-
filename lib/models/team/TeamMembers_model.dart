...
class TeamMemberModel with TopModel {
  ...
  set setId(String idParameter) {
    ...
    if (idParameter.isEmpty) {
      exception = Exception("team members id cannot be empty");
      throw exception;
    }
    id = idParameter;
  }
  ...
  set setCreatedAt(DateTime createdAtParameter) {
    ...
    if (createdAtParameter.isAfter(now)) {
      exception =
          Exception("team member creating time cannot be in the future");
      throw exception;
    }
    //تاريخ إضافةالدوكيومنت الخاص بالعضو لا يمكن أن يكون قبل الوقت الحالي
    if (firebaseTime(createdAtParameter).isBefore(now)) {
      exception = Exception("team member creating time cannot be in the past");
      throw exception;
    }
    createdAt = createdAtParameter;
  }
